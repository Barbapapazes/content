...
import { type PropType, type VNode, defineComponent, h, toRefs, useSlots, watch } from 'vue'
import type { ParsedContent, ParsedContentMeta, QueryBuilder, SortParams } from '../types'
import { computed, queryContent, useAsyncData, useRuntimeConfig } from '#imports'
  ...
  async setup(props) {
    ...
    const { data, refresh } = await useAsyncData<ParsedContent | ParsedContent[] | [ParsedContent | undefined, ParsedContent | undefined]>(
      ...
      () => {
        ...
        if (only.value)
          queryBuilder = queryBuilder.only(only.value) as QueryBuilder<ParsedContentMeta>

        if (without.value)
          queryBuilder = queryBuilder.without(without.value) as QueryBuilder<ParsedContentMeta>
        ...
        if (find.value === 'one')
          return queryBuilder.findOne().then(resolveResult) as Promise<ParsedContent>
